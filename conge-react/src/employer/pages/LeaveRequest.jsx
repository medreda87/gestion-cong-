...
const LeaveRequest = () => {
  ...
  const { getEmployeeTotalBalance, deductLeaveDays } = useBalance();
  const navigate = useNavigate(); 

  const currentBalance = user ? getEmployeeTotalBalance(user.id) : 0;
  const { user } = useAuth();
  const { addRequest } = useLeave();
  const navigate = useNavigate();
  ...
  const total=totall()
  const isValidDuration = (type !== 'exceptional' ? duration > 0 && duration <= user.leaveBalance : duration > 0);
  ...
  const handleSubmit = async (e) => {
    ...
    addRequest({
      ...
      endDate,
      duration,
      subType: type === 'exceptional' ? subType : undefined,
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-primary">{user.leaveBalance} jours</p>
            </div>
            {duration > 0 && type !== 'exceptional' && (
              <div className="text-right">
                ...
                <p className={`text-2xl font-bold ${isValidDuration ? 'text-success' : 'text-destructive'}`}>
                  {user.leaveBalance - duration} jours
                </p>
              </div>
            )}
          </div>
